                        ...
                        if gesture_detection:
                            ...
                            if detected_gesture >= 50:
                                ...
                                if user_result == "commence":
                                    ...
                                    left_hip = (float(keypoints_data.xy[0, 11, 0]), float(keypoints_data.xy[0, 11, 1]))
                                    right_hip = (float(keypoints_data.xy[0, 12, 0]), float(keypoints_data.xy[0, 12, 1]))
...
async def main(service_config_path: Path) -> None:
    # Load a list of service configs and create clients for services with ports.
    ...
    subscriptions = []
    for config in service_config_list.configs:
        if config.port:
            clients[config.name] = EventClient(config)
        else:
            subscriptions = list(config.subscriptions)

    # Choose specific clients/configs
    canbus_client = clients.get("canbus") if "canbus" in clients else None
    filter_client = clients.get("filter") if "filter" in clients else None
    track_follower_client = clients.get("track_follower") if "track_follower" in clients else None

    # Prefer track_follower for movement, otherwise fall back to filter or any available config
    movement_config = next((c for c in service_config_list.configs if c.name == "track_follower" or c.name == "track-follower"), None)
    if movement_config is None:
        movement_config = next((c for c in service_config_list.configs if c.name == "filter"), None)
    if movement_config is None:
        movement_config = next((c for c in service_config_list.configs if c.port), None)

    if canbus_client is None and filter_client is None and track_follower_client is None:
        raise RuntimeError("No service configs found")
    ...
    cam_thread = asyncio.create_task(camera_thread(canbus_client, filter_client, track_follower_client, movement_config))
    await asyncio.sleep(2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--service-config", type=Path, required=True, help="The collective service configs.")
